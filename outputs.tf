...
output "cosmosdb_sql_containers_autoscale_settings" {
  ...
  value       = { for k, v in azurerm_cosmosdb_sql_container.cosmosdb_sql_containers : k => one(v.autoscale_settings) if v.autoscale_settings != null && length(v.autoscale_settings) > 0 }
}
output "cosmosdb_sql_containers_conflict_resolution_policy" {
  ...
  value       = { for k, v in azurerm_cosmosdb_sql_container.cosmosdb_sql_containers : k => one(v.conflict_resolution_policy) if v.conflict_resolution_policy != null && length(v.conflict_resolution_policy) > 0 }
}
...
output "cosmosdb_sql_containers_indexing_policy" {
  ...
  value       = { for k, v in azurerm_cosmosdb_sql_container.cosmosdb_sql_containers : k => one(v.indexing_policy) if v.indexing_policy != null && length(v.indexing_policy) > 0 }
}
